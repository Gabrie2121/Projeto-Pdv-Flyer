...
function getValues() {
  ...
  valor_total.value = (
    parseFloat(jsonsession.vTotal).toFixed(2) -
    parseFloat(jsonsession.desconto).toFixed(2)
  ).toFixed(2);
  return jsonsession;
}
getValues();
...
const calcValor = () => {
  valor_troco.value = (
    parseFloat(-valor_total.value) + parseFloat(valor_recebido.value)
  ).toFixed(2);
};
...
const venda = (valor) => {
  ...
  console.log(jsonsession.codcomanda);
  try {
    ...
    fetch(`http://31.220.21.132:5000/comanda/${jsonsession.codcomanda}`, {
      method: "DELETE",
    })
      .then((res) => res.text()) // or res.json()
      .then((res) => window.location.href = "finalizada.html")
